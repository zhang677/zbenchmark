...
def coarsen(A, levels):
    ...
    print("coarsening.py levels : ",levels)
    for i in range(len(graphs)):
        print("coarsening.py HEM graph[%d] : "%(i),graphs[i].nnz)
    for i in range(len(parents)):
        print("coarsening.py HEM parents[%d] : "%(i),len(parents[i]))
    ...
    for i  in range(len(perms)):
        print('coarsening.py perms[%d]'%(i),len(perms[i]))
...
def compute_perm(parents):
    ...
    for parent in parents[::-1]: #[::-1]表示从后向前取
