...
def z2polar(edges):
    z = edges.dst['xy'] - edges.src['xy'] # 直角坐标差
    rho = th.norm(z, dim=-1, p=2)
    x, y = z.unbind(dim=-1) # x存坐标x的tensor，y存坐标y的tensor
    phi = th.atan2(y, x)
    return {'u': th.cat([rho.unsqueeze(-1), phi.unsqueeze(-1)], -1)} # 边上存的是相邻node的欧式距离

def cheb_z2_dist(edges):
    z = edges.dst['xy'] - edges.src['xy'] # 直角坐标差
    rho = th.norm(z, dim=-1, p=2)
    return {'v': rho}
